...
import { FaPrint, FaTimes, FaCheck, FaCalendar, FaClock ,FaArrowLeft } from "react-icons/fa";
import { FaEdit, FaArrowsAlt, FaCodeBranch, FaExchangeAlt,} from "react-icons/fa";
...
const Header = () => {
    ...
    return (
      <div>
        <div className="flex justify-between items-center py-4 px-6 bg-white border-b" style={{ width: '67%' }}>
        ...
        >
          <div className="bg-white w-full max-w-md mx-auto p-6 rounded-lg shadow-lg">
            ...
            {/* Select Waiter */}
            <div className="mt-4">
              <label className="block text-sm font-medium mb-2">Select Waiter</label>
...
const OrderItem = ({ order ,onSelect,selected}) => {
  ...
  return (
    <div   className={`relative p-3 mb-3 rounded-lg shadow-md flex justify-between items-center border cursor-pointer
        ${
          selected
     ...
     onClick={() => onSelect(order)} >
      <div className="flex flex-1 items-center">
        <div className="ml-4">
          <h3 className="text-lg font-semibold">Quantity: {order.items.length}</h3>
          <p className="text-sm">Total Amount: ${totalAmount.toFixed(2)}</p>
          <p className="text-sm">Order Created: {new Date(order.OdercreateDate).toLocaleString()}</p>
        </div>
      </div>
...
const OrderDetails = ({ order }) => {
    ...
    return (
          ...
          <div className="mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <p>{order.priceCategory}</p> {/* Assuming priceCategory is part of the order */}
              </div>
              <div>
        ...
        {order.items.map((item, index) => (
          <div key={item._id} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <p>{index + 1}</p> {/* Serial Number */}
            <p>{item.itemName}</p>
            <p>{item.quantity}</p>
            <p>{item.notes || "N/A"}</p> {/* Assuming status can be represented with notes or set to "N/A" if empty */}
          </div>
        ))}
      </div>
    </div>
            ...
            {orders.map((order) => (
              <div key={order?._id} className="mb-2">
                {order?.items.map(item => (
                    <div key={item.id} className="flex justify-between items-center py-2 px-4 bg-white text-black rounded-lg mb-2 border border-gray-300">
...
const SalesSection = () => {
    const { tableId } = useParams(); // Get tableIds from URL params
  ...
  useEffect(() => {
    const fetchWaiterOrders = async () => {
      try {
        const response = await axios.get(`https://tyem.invenro.com/api/user/${tableId}/getWaiterOrder`);
        console.log(response);

        // No need to call response.json(), just access response.data
        setWaiterOrders(response.data);
      } catch (error) {
    ...
    fetchWaiterOrders();
  }, [tableId]); // Add tableId as a dependency if it changes

  const handleOrderSelect = (order) => {
    setSelectedOrder(order); // Update the selected order
  };
    ...
    return (
      <div className="flex flex-col h-screen">
        ...
        <div className="flex flex-grow">
          {/* Order List Section */}
          <div id="order-list" className="w-full md:w-1/3 h-full p-4 border-r border-gray-300 bg-white overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-100">
          ...
          <div className="hidden md:block w-full md:w-1/3 h-full p-4 bg-white overflow-auto">
            <OrderDetails order={selectedOrder} /> {/* Assuming you want to display the first order details */}
          </div>
  
          {/* Cart Section */}
          <div className="hidden md:block w-full md:w-1/3 h-full p-4 border-l border-gray-300 bg-white" style={{ marginTop: "-74px" }}>
            <CartSection orders={[selectedOrder]} />
          </div>
        </div>
      </div>
    );
